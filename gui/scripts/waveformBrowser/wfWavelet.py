...
def graph(fileName, table, window):
    ...
    button_back = customtkinter.CTkButton(wfNormal.plotFrame, text="<<", command=lambda: wfNormal.back(numOfImages-1, cDs,
         window, numOfImages, time, dontTransform, peaks, channels))
    button_next = customtkinter.CTkButton(wfNormal.plotFrame, text=">>", command=lambda: wfNormal.next(1, cDs, 
        window, numOfImages, time, dontTransform, peaks, channels))
