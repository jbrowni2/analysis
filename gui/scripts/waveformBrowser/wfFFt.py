...
def graph(fileName, table, window):
    ...
    fourier = np.array([[0]*len(t1_data[0]["waveform"]["values"].nda[0])]*len(t1_data[0]["waveform"]["values"].nda[0]))
    for i in range(0,len(t1_data[0]["waveform"]["values"].nda)):
        fftHold = np.abs(np.fft.fft(t1_data[0]["waveform"]["values"].nda[i]))**2
        fourier[i] = fftHold
        
            
    df = pd.DataFrame(fourier[0]).T
    dontTransform = False
    ...
    button_back = customtkinter.CTkButton(wfNormal.plotFrame, text="<<", command=lambda: wfNormal.back(numOfImages-1, fourier,
         window, numOfImages, time, dontTransform, peaks, channels))
    button_next = customtkinter.CTkButton(wfNormal.plotFrame, text=">>", command=lambda: wfNormal.next(1, fourier, 
        window, numOfImages, time, dontTransform, peaks, channels))
